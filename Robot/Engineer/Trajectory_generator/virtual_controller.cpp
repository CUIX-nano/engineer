...
void VirtualController::buildCheckpoints() {
    ...
    // 由于 Action 没有直接暴露 Pose 列表，我们需要从 Action 中提取校验点信息
    // 但 Action 目前只存储系数表，不存储原始 Pose。
    // 因此有两种方案：
    //   方案 A：在 Action 中添加 getCheckpoints() 接口
    //   方案 B：在注册时把校验点信息单独存一份
    // 这里采用方案 A 的变体：通过 getTotalDuration() 和假设校验点均匀分布，
    // 实际使用时，应该由 Action 提供 getCheckpoints() 接口。
    // 
    // 为了保持代码可编译，这里暂时留空，实际使用时需要从 Action 获取。
    // 这里给出一个示例实现：如果 Action 提供了一个 getCheckpointAt(index) 方法。
    // 
    // 临时实现：从 Action 中获取校验点（需要扩展 Action 接口）
    // 作为替代，我们可以使用模板特化或外部注入，但这里先保持简单。
    // 
    // 注意：这是一个设计决策点，后续可能需要扩展 Action 类。
    // 目前先留空，让用户通过外部方式提供校验点。
    // 
    // 更好的做法：在 loadAction 时，通过 Action 提供的一个新接口获取。
    // 由于 Action 目前没有此接口，我们暂时跳过校验点构建。
    // 
    // 实际生产环境中，可以修改 Action 添加：
    //   size_t getCheckpointCount() const;
    //   Checkpoint getCheckpoint(size_t idx) const;
    // 
    // 这里我们暂时注释掉，并用一个占位。
    // 
    // 如果不希望修改 Action，也可以将校验点列表作为外部参数传入 VirtualController。
    // 
    // 为了演示，我们在这里留空，实际使用时需要根据实际情况实现。
    // 
    // TODO: 扩展 Action 接口以提供校验点列表
}
...
bool VirtualController::isAtCheckpoint(double time) const {
    ...
    for (size_t i = 0; i < num_checkpoints_; ++i) {
        if (std::abs(time - checkpoints_[i].time) < EPS) {
            return true;
        }
    }
    return false;
...
double VirtualController::getCurrentCheckpointTolerance() const {
    for (size_t i = 0; i < num_checkpoints_; ++i) {
        if (std::abs(current_time_ - checkpoints_[i].time) < 1e-6) {
